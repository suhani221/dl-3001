#modified model.py to test out the updated adjacency matrix with semantic embeddings with DCRNN (SOTA). Didn't work out. On discussions with Suhani - we realised that we needed a Graph Constructor for DCRNN to be able to process the adjacency matrix like NeuroGNN, which proved to be too complex to do in addition to carrying on all the other experiments.

# also performed baseline analysis with Conv1d and Conv2D of HMS Kaggle EEG dataset that we were exploring before TUSZ dataset but didn't upload to avoid confusion.
...
class DCRNNEncoder(nn.Module):
    def __init__(self, input_dim, max_diffusion_step, hid_dim, num_nodes, num_rnn_layers, dcgru_activation=None, filter_type='laplacian', device=None):
    ...
    def forward(self, inputs, initial_hidden_state, supports):
        ...
        current_inputs = inputs.view(seq_length, batch_size, -1)
        output_hidden = []
        ...
        return torch.stack(output_hidden, dim=0).to(self._device), current_inputs
...
class DCRNNModel_classification(nn.Module):
    def __init__(self, args, num_classes, device=None, adj_matrix=None, semantic_embs=None, meta_node_indices=None):
        super(DCRNNModel_classification, self).__init__()

        self.num_nodes = args.num_nodes  # Assumes 38 if using an extended matrix
        ...
        self.adj_matrix = torch.tensor(adj_matrix, dtype=torch.float32, device=device)
        self.attention_weights = nn.Parameter(torch.randn(self.num_nodes, self.num_nodes))
        nn.init.xavier_uniform_(self.attention_weights.data, gain=1.414)

        # Store semantic embeddings and meta node indices
        self.semantic_embs = torch.tensor(semantic_embs, dtype=torch.float32, device=device) if semantic_embs is not None else None
        ...
        self.fc = nn.Linear(self.rnn_units, num_classes)
    ...
    def forward(self, input_seq, seq_lengths):
        ...
        attention_scores = torch.softmax(torch.matmul(self.adj_matrix, self.attention_weights), dim=-1)
        input_seq = torch.matmul(attention_scores, input_seq.transpose(1, 2)).transpose(1, 2)

        input_seq = torch.transpose(input_seq, 0, 1)
        supports = self._build_supports(self.adj_matrix, args.filter_type, args.max_diffusion_step)
        ...
        node_embeds_detached = last_out.detach().cpu().numpy()
        last_out = last_out.to(self._device)

        if self.semantic_embs is not None:
            # Optionally enrich node features with semantic embeddings
            semantic_features = self.semantic_embs.unsqueeze(0).expand(batch_size, -1, -1)
            last_out = torch.cat((last_out, semantic_features), dim=2)
    ...
    @staticmethod
    def _build_supports(adj_matrix, filter_type, max_diffusion_step):
        if filter_type == 'laplacian':
            return [utils.laplacian(adj_matrix)]
        elif filter_type == 'random_walk':
            return [utils.random_walk_matrix(adj_matrix)]
        else:
            return [adj_matrix]
########## Model for seizure classification/detection ##########
