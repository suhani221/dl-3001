...
"""
### Model.py with optional temporal attention mechanism 
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        query = self.query_layer(hidden_states[-1])  # Last state as query
        keys = self.key_layer(hidden_states)  # All states as keys
        values = self.value_layer(hidden_states)  # All states as values

        attention_scores = torch.bmm(query.unsqueeze(1), keys.transpose(1, 2))
        attention_weights = self.softmax(attention_scores)
        context_vector = torch.bmm(attention_weights, values).squeeze(1)
...
class DCRNNModel_classification(nn.Module):
    def __init__(self, args, num_classes, device=None):
        ...
        self.encoder = DCRNNEncoder(input_dim=args.input_dim,
                                    max_diffusion_step=args.max_diffusion_step,
                                    hid_dim=args.rnn_units, num_nodes=args.num_nodes,
                                    num_rnn_layers=args.num_rnn_layers,
                                    dcgru_activation=args.dcgru_activation,
                                    filter_type=args.filter_type)
        self.temporal_attention = TemporalAttention(args.rnn_units * args.num_nodes)  # Adjust size if needed
        self.fc = nn.Linear(args.rnn_units * args.num_nodes, num_classes)  # Adjust according to attention output
    ...
    def forward(self, input_seq, seq_lengths, supports):
        ...
        input_seq = input_seq.permute(1, 0, 2, 3)  # (seq_len, batch, num_nodes, input_dim)
        input_seq = input_seq.reshape(input_seq.shape[0], input_seq.shape[1], -1)  # (seq_len, batch, num_nodes*input_dim)

        # Initialize hidden state and encode
        ...
        final_hidden = final_hidden.permute(1, 0, 2)  # (batch, seq_len, num_nodes*rnn_units)
        context_vector = self.temporal_attention(final_hidden)

        # Classify from the context vector
        context_vector = self.dropout(self.relu(context_vector))
        pool_logits = self.fc(context_vector)  # (batch_size, num_classes)
