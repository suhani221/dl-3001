...
"""
#### DCRNN with spatial attention mechanism 
...
class DiffusionGraphConv(nn.Module):
    def __init__(self, num_supports, input_dim, hid_dim, num_nodes,
                 ...
                 filter_type='laplacian', use_attention=True):
    ...
    def forward(self, supports, inputs, state, output_size, bias_start=0.0):
        # Reshape input and state to (batch_size, num_nodes, input_dim/hidden_dim)
        ...
        if self._max_diffusion_step == 0:
            ...
            for support in supports:
                ...
                if self.use_attention:
                    attn_scores = F.softmax(torch.matmul(inputs, state.transpose(1, 2)), dim=1)
                    # Apply attention to supports
                    supported_inputs = torch.bmm(support, attn_scores)
                else:
                    supported_inputs = inputs  # If not using attention, input remains unchanged

                # (batch, num_nodes, input_dim+hidden_dim)
                x1 = torch.matmul(supported_inputs, x0)
                # (batch, ?(num_diffusion_steps), num_nodes, input_dim+hidden_dim)
                x = self._concat(x, x1)
                for k in range(2, self._max_diffusion_step + 1):
                    # (batch, num_nodes, input_dim+hidden_dim)
                    x2 = 2 * torch.matmul(supported_inputs, x1) - x0
                    x = self._concat(
